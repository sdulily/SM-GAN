...
def main():
  ...
  loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)#这个地方是False 但是train中的本来是true 所以我在测试的时候 所有的顺序都是对的 但是训练什么都学不到
  ...
  #读取之后下面的files ['gate', 'text', 'token', 'attn1', 'attn2']
  # 读取token[list(['[CLS]', 'Re', '##mo', '##ve', 'bottom', 'small', 'purple', 'cube', '.', '[SEP]']),
  #        list(['[CLS]', 'Make', 'right', 'large', 'yellow', 'cube', 'brown', '.', '[SEP]']),……
  #text中就是每个句子的语句
  #读取attn1：(attn1应该是进到where里 )里面是每句话中每个词的权重array([0.        , 0.01522448, 0.19645667, 0.09271105bottom, 0.02842858small,
  #            0.14010656purple, 0.48681155cube, 0.04026106, 0.        ], dtype=float32),
  return
