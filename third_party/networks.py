...
class ContentEncoder(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type):
        ...
        # downsampling blocks
        for i in range(n_downsample):#参数值为2
...
class ResBlock(nn.Module):
    def __init__(self, dim, norm='in', activation='relu', pad_type='zero'):
        ...
        model = []#每一层resbolcks里面都有两层的Con2dBlock
...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
        ...
        self.use_bias = True
        # initialize padding 首先是padding层 边缘填充
        if pad_type == 'reflect':
        ...
        # initialize normalization归一化
        norm_dim = output_dim
        if norm == 'bn':
            self.norm = nn.BatchNorm2d(norm_dim)
        elif norm == 'in':
            #self.norm = nn.InstanceNorm2d(norm_dim, track_running_stats=True)
            self.norm = nn.InstanceNorm2d(norm_dim)#对于一个4D（N, C, H, W）张量，对于每个mini-batch (N)， 在每个通道 ( C ) 对每个二维张量 (H, W) 单独进行计算均值和方差。
                                                   #即对于一个（16，256， 128，128）的张量，计算16*256次均值方差
        elif norm == 'ln':
